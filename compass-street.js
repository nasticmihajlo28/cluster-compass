/* =====================================================================
   ...
   ===================================================================== */
const SETTINGS = {
  version: "street",              // "stage" (keynote) | "street" (shared with the world) — internal names
  ...
  allowBack: true,              // stage: no back buttons, first reaction stands; Street: true
  welcome: {
    showLogo: true,              // David Allison logo at the top of the welcome page
    showIntro: true,            // compass-in-your-pocket setup paragraph (Street; David says it on stage)
    showSiteLink: true,         // quiet davidallisoninc.com link at the bottom of the welcome page (Street)
  ...
  results: {
    ...
    showScenarios: "one",     // "open" | "collapsed" | "one" | "hidden" — hidden on Stage; "one" teases on Street
    showCompassHeadings: true,
    showHeadingsHelp: true,     // how-to-use-the-headings line (Street; David explains it on stage)
    showScoreBars: true,         // "Your Score"
    showRoomSplit: false,        // stage: the room split lives ONLY on David's presenter view
    showEmailCapture: true,
    showFollowBlock: true,      // Street: follow-David block right under the email card
    showShare: true,            // "Share this quiz" button (Street)
  ...
  api: {
    ...
    source: "street",                  // "" for the Stage build; "street" tags responses so they never
...
(function () {
  ...
  // cold backend never turns into a visible error for the first person to arrive.
  ...
  /* ---------- boot ---------- */
  mountMotif();
  warmUp(); // wake the backend on page load, not at the email form
